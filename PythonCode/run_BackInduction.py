# -*- coding: utf-8 -*-
"""
Created on Mon Jun  7 01:57:07 2021
...
def getCurrentIndexNumFromValue(BN, AllStateSet):
    ...
    CurrentIndexNum = 0
    CurrentOptIndex = 0
    TempRecord_Thres = 1000  # hyperparameter
    for ii in Sequence:
        i = ii[0]
        CurrentIndexNum = CurrentIndexNum + 1   
...
def update_ValueFunctionWithStartIndex(BN, AllStateSet, StartIndex):
    ...
    for ii in Sequence:
        ...
        if Signal % 50000 == 0:
            FileName = 'DP_Record_MidNet_0727_'+str(Signal)+'_bakupdata_1.out'
            AllStateSet.saveAllStates(FileName)
            print('Saved for:',str(Signal))
...
if __name__ == "__main__":
    ...
    TotalNum = update_AllOrders(BN, AllStateSet)
    ...
    # StartIndex3 = 719600
    # MaxError,AllStateSet,Signal = update_ValueFunctionWithStartIndex(BN,AllStateSet,StartIndex3)
    ...
    # DataFileName = 'DP_TestData_MidNet_0608_ordered_1.out'
    DataFileName = 'DP_TestData_MidNet_0730_2.out'
    Dict = {'AllStateValue' : AllStateSet.AllStateValues, 'VACost': BN.VACost, 'CACost': BN.CACost\
            ...
            , 'ActionTree': ActionTree, 'ListOfOptimalTree' : ListOfOptimalTree, 'TotalNum' : TotalNum\
            , 'FailCost': BN.FailCost, 'RepairLRVector': BN.RepairLRVector, 'start': start, 'mid': mid, 'end': end}
