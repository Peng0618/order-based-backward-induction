# -*- coding: utf-8 -*-
"""
Created on Tue Jun  8 16:52:48 2021
...
class AllStates4ThresRule(object):
    ...
    def __init__(self, **kw):   
        ...
        self.StateCount = pow(3,(self.VEN-self.VSN+1))
    
        self.AllStateValues = np.zeros((self.StateCount,1))
        ...
        self.AllStateActions = np.zeros((self.StateCount,1))
        ...
        if CAOrNot == 0:
            if Action == -1 or Action == -2:
                ...
                if PostProb <= self.BN.LowerThres:        
                    # modified at 0727:
                    ParameterOfTheAction = Action-(self.VSN-1)    # need check!!!
                    
                    ChildList = self.BN.findAllChildNode(ParameterOfTheAction)
                    for i in ChildList:
                        if i >= (self.BN.VSN-1):
                            Key_2[i] = 0
